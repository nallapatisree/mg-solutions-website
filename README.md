...
│   │   ├── gallery/              # Public photo gallery, fed by Admin → Media
...
| `media` | Uploaded file registry (`is_visible` controls public Gallery listing) | — |
...
8. Upload a photo in **Admin → Media** → it appears at `/gallery` immediately, marked "In Gallery". Click any photo in the Media Library grid to hide it from the public page without deleting the file — useful for icons/logos you uploaded for internal use only.

## 10. Where uploaded images appear

Every image uploaded anywhere in the CMS — a service icon, a project screenshot, a testimonial photo, or a direct upload — is recorded once in the `media` table and listed at **Admin → Media**. That's the single place to browse everything that has ever been uploaded, copy its URL, or delete it.

Separately, the public **`/gallery`** page (linked in the main navigation) shows a filtered, visitor-facing subset: only files marked "In Gallery." New uploads default to visible, so nothing extra needs to be done — but if you upload something you don't want the public to see (an internal-only icon, a draft image), open Media Library and click it once to hide it. The toggle is instant and doesn't delete the file or affect where else it's used on the site.
