...
// Toggles whether a file appears on the public Gallery page.
export async function PATCH(request) {
  ...
  try {
    ...
    const db = getDb();
    const row = db.prepare('SELECT id FROM media WHERE id = ?').get(id);
    if (!row) return NextResponse.json({ error: 'Not found' }, { status: 404 });

    db.prepare('UPDATE media SET is_visible = ? WHERE id = ?').run(is_visible ? 1 : 0, id);
    logActivity(db, session.email, is_visible ? 'Showed media in gallery' : 'Hid media from gallery', 'media', id);
